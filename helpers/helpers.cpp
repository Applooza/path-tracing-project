...
SDL_Window* init_window_SDL(std::string caption, int width, int height)
{
	...
	helpers::startupGLDiagnostics();
	helpers::setupGLDebugMessages();
                   ...
                   const glm::vec3& worldSpaceLightPos)
{
	...
	if(vertexArrayObject == 0)
	{
		...
		static const glm::vec3 positions[] = { { worldSpaceLightPos.x, worldSpaceLightPos.y,
			                                   ...
			                                   { 0.0f, 0.0f, 0.0f } };
		helpers::createAddAttribBuffer(vertexArrayObject, positions, sizeof(positions), 0, 3, GL_FLOAT);
	}
	glBindVertexArray(vertexArrayObject);
...
void drawFullScreenQuad()
{
	...
	if(vertexArrayObject == 0)
	{
		...
		helpers::createAddAttribBuffer(vertexArrayObject, positions, sizeof(positions), 0, 2, GL_FLOAT);
	}
	glBindVertexArray(vertexArrayObject);
